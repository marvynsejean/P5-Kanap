...
let idPageProduct = window.location.href;
let url = new URL(idPageProduct);
let idProduct = url.searchParams.get("id");
console.log("idProduct");
const urlProduct = catalogue + "/" + idProduct;
let article = "";
...
let productObject = (product) => {
  ...
  button.addEventListener("click", (event) => {
    ...
    if (selectedColor.value == false) {
      ...
      if (existingCart) {
        console.log(
          "Il y a déjà un produit dans le panier, on compare les données"
        );
